...
- ⚠️ 마이그레이션 0017은 **사용자가 직접 프로덕션 `supabase db push`**(코드 병합보다 먼저). 구현·검증은 로컬에서.
...
### Task 1: 마이그레이션 0017 (컬럼 + 공개읽기 RLS + RPC 재작성)

**Files:**
- Create: `supabase/migrations/0017_requested_room_type.sql`
...
`supabase/migrations/0017_requested_room_type.sql`:
...
```bash
git add supabase/migrations/0017_requested_room_type.sql
git commit -m "feat(db): requested_room_type_id + room_types 공개읽기 + my_household_fee 선택기반(0017)"
```
...
- ⚠️ **프로덕션 `supabase db push`로 0017 적용**(코드 병합/배포보다 먼저).
