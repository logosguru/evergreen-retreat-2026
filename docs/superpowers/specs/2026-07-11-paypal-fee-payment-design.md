...
| 금액 | **사전입력**. 인라인 페이지에서 고정 표시됨(=성도가 입력 불필요, OK) |
...
## 기술 근거 (PayPal 공식 문서 + 실제 브라우저 검증, 2026-07-11)

- 인라인 donate URL은 `business`, `amount`, `currency_code`, `item_name`,
  `item_number`, `no_recurring` 변수를 지원한다.
- **실제 검증**: 교회 이메일(`newyorkevergreen@gmail.com`)로 인라인 URL을
  브라우저에서 열어본 결과 —
  - 기존 hosted button과 **동일한 브랜딩**("Donate to The Evergreen Presbyterian
    Church", 교회 로고)으로 렌더됨.
  - `amount=300` 이 **$300 USD 로 정상 반영**(=계정이 인라인 결제를 차단하지 않음).
  - `item_name`("Evergreen Retreat 2026 Fee")·`item_number`("Test (District 1)")가
    페이지에 표시됨 → 무엇을·누가 내는지 노출, 대조 가능.
  - 인라인 `amount` 는 **고정 표시**(입력란 없음) — 성도가 금액 입력 불필요.
  - hosted button 대비 프리셋($25/$50/$100)·반복헌금·용도 드롭다운이 없어 회비
    납부엔 오히려 더 단순함.
- `item_number` 는 거래내역에 남아 **대조용 참조값**으로 쓸 수 있다.
- ⚠️ 브라우저 렌더 검증은 완료. **교회 계정 실결제 1건**으로 거래내역에
  `item_number` 가 실제로 남는지 최종 확인 권장(사용자).
- 기존 `hosted_button_id` 링크는 설정이 서버 저장이라 URL 파라미터(금액·참조값)가
  무시되므로 인라인 방식을 사용한다.
...
- head 이름 공백 → `english_name` 폴백, 둘 다 없으면 `item_number` 는 구역만/빈값
  허용(빌더는 방어적으로 처리).
- 인라인 금액 반영은 브라우저에서 확인됨($300 표시). 거래내역 `item_number` 노출은
  실결제 1건으로 최종 확인(사용자).
